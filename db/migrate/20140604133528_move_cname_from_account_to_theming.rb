class MoveCnameFromAccountToTheming < ActiveRecord::Migration
  def up
    ...
    execute("UPDATE pageflow_themings SET cname = (SELECT DISTINCT cname FROM pageflow_accounts WHERE pageflow_accounts.default_theming_id = pageflow_themings.id);")
  ...
  def down
    ...
    execute("UPDATE pageflow_accounts SET cname = (SELECT DISTINCT cname FROM pageflow_themings WHERE pageflow_themings.id = pageflow_accounts.default_theming_id);")
