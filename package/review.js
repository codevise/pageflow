...
class ReviewSession {
    ...
    const thread = await this._request({
      ...
      payload: {
        comment_thread: {
          ...
          ...(sectionPermaId != null && {
            section_perma_id: sectionPermaId
          }),
          comment: {
            body,
            ...(quote && {
  ...
  findThreadsFor({
    ...
    if (!this._state) return [];
    return this._state.commentThreads.filter(thread => thread.subjectType === subjectType && thread.subjectId === subjectId);
  }
  async fetch() {
  ...
  async flushReads() {
    ...
    await this._request({
      ...
      method: 'POST',
      payload: {
        comment_thread_perma_ids: permaIds
      }
    }).catch(() => {
  ...
  _findThread(id) {
    var _this$_state;
    return (_this$_state = this._state) === null || _this$_state === void 0 ? void 0 : _this$_state.commentThreads.find(t => t.id === id);
  }
  _upsertThread(thread) {
    ...
    if (index >= 0) {
      this._state = {
        ...this._state,
        commentThreads: [...threads.slice(0, index), thread, ...threads.slice(index + 1)]
      };
    } else {
      this._state = {
        ...this._state,
...
export { ReviewSession, createReviewSession };
