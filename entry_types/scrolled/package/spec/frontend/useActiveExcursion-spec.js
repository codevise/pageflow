...
const mockScrollToTarget = jest.fn();

jest.mock('frontend/useScrollTarget', () => ({
  ...jest.requireActual('frontend/useScrollTarget'),
  useScrollToTarget: () => mockScrollToTarget
}));

describe('ActiveExcursionProvider', () => {
  beforeEach(() => {
    window.location.hash = '#initial';
    mockScrollToTarget.mockClear();
  });

  it('returns undefined by default', () => {
    const {result} = renderHookInEntry(() => useActiveExcursion(), {
      seed: {
      }
    });
  ...
  describe('scrolling', () => {
    ...
    it('calls scrollToTarget when navigating from main to excursion section', async () => {
      ...
      const {activeExcursion} = result.current;
      expect(activeExcursion).toMatchObject({title: 'excursion'});
      expect(mockScrollToTarget).toHaveBeenCalledWith({id: 102});
    });
    ...
    it('calls scrollToTarget when navigating from excursion to main section', async () => {
      ...
      const {activeExcursion} = result.current;
      expect(activeExcursion).toBeUndefined();
      expect(mockScrollToTarget).toHaveBeenCalledWith({id: 100});
    });
    ...
    it('calls scrollToTarget when navigating from excursion A to excursion B', async () => {
      ...
      expect(mockScrollToTarget).toHaveBeenCalledWith({id: 103});
    });

    it('delays scroll by 500ms', async () => {
      renderHookInEntry(() => useActiveExcursion(), {
        seed: {
          storylines: [
            {id: 1, configuration: {main: true}},
            {id: 2}
