  ...
  render() {
    ...
    return (
      <div className={wrapperClassName(props, this.state.subMenuVisible)}
           ...
           onBlur={this.onBlur}>
        <a className={styles.text_track_button_link}
           href="#"
           tabIndex="4"
           title={props.title}
...
function renderSubMenu(props, closeMenu) {
  if (props.subMenuItems.length > 0) {
    return (
      <ul className={classNames("player_controls-menu_bar_button_sub_menu", styles.text_track_button_submenu)}>
...
function renderSubMenuItems(props, closeMenu) {
  return props.subMenuItems.map(item => {
    return (
      <li className={classNames(itemClassName(item),styles.text_track_menu_item)} key={item.value}>
        <a className={classNames(styles.text_track_menu_item_link)}
           href="#"
           tabIndex="4"
...
function wrapperClassName(props, subMenuVisible) {
  return classNames({[styles.text_track_submenu_visible]: subMenuVisible},
                    className(props));
}
...
function renderSubMenuItemIcon(item) {
  if (item.active) {
    return (
      <Icon className={styles.text_track_menu_item_active}
            name="activeMenuItem" />
    );
  }
}
