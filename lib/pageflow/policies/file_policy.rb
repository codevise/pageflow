  ...
  module Policies
    class FilePolicy
      ...
      def previewer_of_any_entry_using_file_or_its_account?(user, file)
        entries_user_is_previewer_or_above_on = Pageflow::Policies::EntryPolicy::Scope.new(user, Entry).resolve
      ...
      def editor_of_any_entry_using_file_or_its_account?(user, file)
        entries_user_is_editor_or_above_on = Pageflow::Policies::EntryPolicy::Scope.new(user, Entry).editor_or_above
